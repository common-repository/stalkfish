 ...
 */
namespace Stalkfish\Dependencies\Symfony\Component\VarDumper\Caster;

use Stalkfish\Dependencies\Symfony\Component\HttpFoundation\Request;
use Stalkfish\Dependencies\Symfony\Component\Uid\Ulid;
use Stalkfish\Dependencies\Symfony\Component\Uid\Uuid;
use Stalkfish\Dependencies\Symfony\Component\VarDumper\Cloner\Stub;
/**
 * @final
 */
class SymfonyCaster
{
    ...
    public static function castUuid(Uuid $uuid, array $a, Stub $stub, bool $isNested)
    {
        ...
        if (\method_exists($uuid, 'getDateTime')) {
            $a[Caster::PREFIX_VIRTUAL . 'time'] = $uuid->getDateTime()->format('Stalkfish\\Dependencies\\Y-m-d H:i:s.u \\U\\T\\C');
        }
        return $a;
    }
    public static function castUlid(Ulid $ulid, array $a, Stub $stub, bool $isNested)
    {
        ...
        if (\method_exists($ulid, 'getDateTime')) {
            $a[Caster::PREFIX_VIRTUAL . 'time'] = $ulid->getDateTime()->format('Stalkfish\\Dependencies\\Y-m-d H:i:s.v \\U\\T\\C');
        }
        return $a;
    }
}
